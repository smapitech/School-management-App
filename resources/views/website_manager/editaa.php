...
<form class="website-pro-editor" method="post" action="/website-manager/save" enctype="multipart/form-data">
    ...
    <div class="website-pro-editor-groups">
        <?php
        $groupDescriptions = [
            'Brand & SEO' => 'Set the identity displayed in browsers, search results and social sharing.',
            'Hero Slideshow' => 'Manage up to three large homepage banners. Empty slides are automatically hidden.',
            'About, Mission & Vision' => 'Tell your school story and explain the purpose behind your work.',
            'Homepage Highlights' => 'Present your strengths, programmes, gallery and teaching team.',
            'Contact Information' => 'Make it easy for families to reach the school office.',
            'Social Media' => 'Connect your public website to the school’s official social channels.',
        ];
        ?>
        <?php foreach ($fieldGroups as $groupIndex => $fields): ?>
        <section id="website-group-<?= $groupIndex + 1 ?>" class="website-pro-editor-group"><header><span><?= str_pad((string) ($groupIndex + 1), 2, '0', STR_PAD_LEFT) ?></span><div><h3><?= e($groupIndex) ?></h3><p><?= e($groupDescriptions[$groupIndex] ?? '') ?></p></div></header><div class="website-pro-editor-grid">
            <?php foreach ($fields as $field): ?>
                ...
                <label class="<?= $isWide ? 'is-wide' : '' ?>"><span><?= e($labels[$field] ?? ucwords(str_replace('_', ' ', $field))) ?></span>
                    ...
                    <?php else: ?>
                        <input name="<?= e($field) ?>" value="<?= e($value) ?>" <?= substr($field, -4) === '_url' ? 'type="url"' : 'type="text"' ?>>
                    <?php endif; ?>
